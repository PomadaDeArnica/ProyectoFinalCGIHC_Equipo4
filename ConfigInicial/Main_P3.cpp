...
int main() {
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Proyecciones y transformaciones basicas // Rodríguez Montes de Oca Andrés", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate(model, 45.5f, glm::vec3(0.0f, 0.0f, 1.0f)); // use to compare orthographic and perspective projection
		...
		model = glm::rotate(model, 45.0f, glm::vec3(8.0f, 0.0f, 1.0f)); // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(1.0f, 4.0f, 3.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		//N
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-3.5f, 2.0f, 0.0f));
		model = glm::rotate(model, 45.5f, glm::vec3(1.0f, 1.0f, 1.5f)); // use to compare orthographic and perspective projection
		...
		model = glm::rotate(model, 180.0f, glm::vec3(1.0f, 1.0f, 0.0f)); // use to compare orthographic and perspective projection
		...
		model = glm::rotate(model, 45.5f, glm::vec3(160.2f,360.0f, 180.0f)); // use to compare orthographic and perspective projection
		...
		model = glm::rotate(model, 45.5f, glm::vec3(3.0f, 1.0f, 6.5f)); // use to compare orthographic and perspective projection
		...
		model = glm::rotate(model, 45.5f, glm::vec3(0.5f, 15.0f, 1.0f)); // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(2.0f, 2.0f, 2.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
